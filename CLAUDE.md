...
- `scripts/notify_email.sh` — reads body from stdin, sends via macOS Mail.app (osascript). Subject is `$1`, optional attachment file path is `$2`. Recipient from `$DIGEST_EMAIL_TO` (default `gian@core-corp.co.jp`).
...
"Update the routine" means: edit `ROUTINE_PROMPT.md`, commit + push to `main`. The next cron run does `git pull --rebase` in the wrapper, so it picks up the new prompt automatically. (Same branch as the digest output — no contention because the wrapper rebases.)
